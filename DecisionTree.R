...
trainindex<-(createDataPartition(mail$V58,p=.8, list= F))

train<- mail[trainindex,]
test<- mail[-trainindex,]

 
###Checking for the distribution of the class in training data.
table(train$class) 
table(test$class)


###variable selection using regression..
...
reults <- as.data.frame(cbind(ptes =ptest, ates = atest, ptar = ptrain, atra = atrain ))

reults$tree <- c(seq(5,9,2),10,seq(11,41,2),42)
names(reults) <- c("ptest","atest","ptrain","atrain","features")

par(mfrow=c(1,2))

p<- ggplot(reults,aes(features)) + 
  geom_line(aes(y=ptrain, color = "ptrain")) +
  geom_line(aes(y=ptest, color = "ptest")) + ggtitle("True positive")

q<- ggplot(reults,aes(features)) + 
  geom_line(aes(y=atrain, color = "atrain")) +
  geom_line(aes(y=atest, color = "atest")) + ggtitle("Accuracy")
