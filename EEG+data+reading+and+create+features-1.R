...
## plotting the training data to see 
...
#pd
t = .9736
sum(LDA_detection[481:504]>t)/24
#0.8333333
#PFA
sum(LDA_detection[1:480]>t)/480
#0.01875
#accuracy 
(sum(LDA_detection[1:480]<t) + sum(LDA_detection[481:504]>t))/504
#0.9781746
#AFAR
ct <- 0 
for(i in 481:504){
  ct[i-481] <- sum(LDA_detection[1:480]>LDA_detection[i])/480
}
sum(ct)/24
#0.004600694
...
#PD
score_mah <- Mahali-Mahalp
sum(score_mah[481:504]>0)/24

#PFA
sum(score_mah[1:480]>0)/480

#Accuracy
(sum(score_mah[1:480]<0) + sum(score_mah[481:504]>0))/504

#AFAR
Afar_mah <- 0 
for(i in 481:504){
  Afar_mah[i-481] <- sum(score_mah[1:480]>score_mah[i])/480
}
sum(Afar_mah)/24
...
t = pm
#z = pd[500,1:192]# if we wnat to try detection against one data values instead of the mean
for (j in 1:504){
...
#0.03125
#accuracy 
(sum(MF[1:480]<t) + sum(MF[481:504]>t))/504
#0.9662698

#AFAR
ct <- 0 
for(i in 481:504){
  ct[i-481] <- sum(MF[1:480]>MF[i])/480
}
sum(ct)/24
...
#pd
t = .20
sum(ACE[481:504]>t)/24
#0.6666667
#PFA
sum(ACE[1:480]>t)/480
#0.00625
#accuracy 
(sum(ACE[1:480]<t) + sum(ACE[481:504]>t))/504
#0.9781

#AFAR
ct <- 0 
for(i in 481:504){
  ct[i-481] <- sum(ACE[1:480]>ACE[i])/480
}
sum(ct)/24
# 0.011
...
#pd
thers <- 0.6303256
sum(probability[481:504]>thers)/24
#0.6666667
#PFA
sum(probability[1:480]>thers)/480
#0.00625
#accuracy 
(sum(probability[1:480]<thers) + sum(probability[481:504]>thers))/504
#0.9781

#AFAR
ct <- 0 
for(i in 481:504){
  ct[i-481] <- sum(probability[1:480]>probability[i])/480
}
sum(ct)/24
# 0.011
